...
public class StardewMinigameStartedEvent extends StardewMinigameEvent {
    ...
    public StardewMinigameStartedEvent(ServerPlayer player, FishingHook hook, ItemStack fishingRod,
        ...
        this.lineStrength = SFAttributes.value(player, SFAttributes.LINE_STRENGTH);
        this.barSize = (int) SFAttributes.value(player, SFAttributes.BAR_SIZE);
        this.treasureChanceBonus = SFAttributes.value(player, SFAttributes.TREASURE_CHANCE_BONUS);
        this.goldenChestBonus = SFAttributes.value(player, SFAttributes.GOLDEN_CHEST_BONUS);
        this.expMultiplier = SFAttributes.value(player, SFAttributes.EXP_MULTIPLIER);
