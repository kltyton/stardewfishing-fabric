...
public class FishingMinigame {
    ...
    public void tick(boolean mouseDown) {
        ...
        if (Math.abs(fishTarget - fishPos) <= Math.abs(fishVelocity)) {
            fishPos = fishTarget;
            fishVelocity = 0;
            fishIsIdle = true;
    ...
    private boolean shouldMoveNow(int idleTicks, Random random) {
        ...
        if (variation > 0 && idleTicks >= packet.idleTime() - variation) {
            return random.nextFloat() <= 1F / variation;
        }
    ...
    private int pickNextTargetPos(int oldPos, Random random) {
        int shortestDistance = Mth.clamp(packet.avgDistance() - packet.moveVariation(), 1, MAX_FISH_HEIGHT);
        int longestDistance = Mth.clamp(packet.avgDistance() + packet.moveVariation(),
                shortestDistance, MAX_FISH_HEIGHT);
